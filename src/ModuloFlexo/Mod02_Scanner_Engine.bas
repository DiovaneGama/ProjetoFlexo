...
Public Sub ExecutarScanner()
    ' [T08] Guard: verifica documento e pagina ativos
    If ActiveDocument Is Nothing Then
        MsgBox "Nenhum documento aberto. Abra um arquivo antes de executar o PreFlight.", _
               vbExclamation, "PreFlight"
        Exit Sub
    End If
    If ActivePage Is Nothing Then
        MsgBox "Nenhuma p" & ChrW(225) & "gina ativa encontrada.", vbExclamation, "PreFlight"
        Exit Sub
    End If
    Dim p As Page
    With relatorio
                ...
                If s.Layer.IsSpecialLayer = False And s.Layer.Printable = True Then
                    ...
                    If s.Outline.Type = cdrOutline Then
                        ...
                        If espMM > 0.005 And espMM <= 0.1 Then
                            ...
                            ' Excecao UNICA: branco entre 0.02mm e 0.05mm = intencional do usuario
                            ' Preenchimento NAO e mais excecao -- deve detectar mesmo assim
                            If EhBranco(s.Outline.Color) Then
                                If Round(espMM, 3) >= 0.02 And Round(espMM, 3) <= 0.05 Then
                                    ehIntencional = True
                                End If
                            End If
                            If Not ehIntencional Then
                                relatorio.QtdLinhasFinas = relatorio.QtdLinhasFinas + 1
                            End If
                        End If
                    ...
                    ElseIf s.Fill.Type = cdrFountainFill Then
                        If s.Locked Or (Not s.Layer.Editable) Then
                            relatorio.QtdGradBloqueado = relatorio.QtdGradBloqueado + 1
                        End If
                        AnalisarGradiente s
                    End If
